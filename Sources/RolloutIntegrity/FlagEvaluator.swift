...
public struct FlagEvaluator: Sendable {
    ...
    /// 6. **Fail-safe variant.**
    ///
    /// The one ordering decision worth defending is (2) above (3): a kill switch
    /// that a sticky pin can outvote is not a kill switch. The cost is that a
    /// user can be un-pinned by a network outage — which is precisely why the
    /// `.experiment` staleness class has no ceiling at all, so experiments never
    /// hit this path and never get their assignment yanked by a tunnel.
    public func evaluate(
        ...
        if let forced = overrides.variant(for: key), definition.containsVariant(named: forced) {
            return FlagDecision(key: key, variant: forced, reason: .localOverride, rulesetVersion: ruleset.version)
        }
        ...
        if let ceiling = definition.stalenessClass.hardCeiling {
            ...
            if age > ceiling {
                return FlagDecision(
                    key: key,
                    variant: failSafe,
                    reason: .staleRulesetFailSafe(ageSeconds: Int(age.rounded()), ceilingSeconds: Int(ceiling)),
                    rulesetVersion: ruleset.version
                )
            }
        }

        // 3. Sticky pin — discarded if the pinned variant no longer exists.
        if definition.pinPolicy == .pinOnFirstExposure,
           ...
           definition.containsVariant(named: pinned.variant) {
            return FlagDecision(
                ...
                reason: .stickyPin(pinnedAtSequence: pinned.pinnedAtSequence),
                rulesetVersion: ruleset.version
            )
        }

        // Buckets are computed once, from `bucketingID` only, and are reused by
        // both the rollout check and the variant split.
        ...
        for rule in definition.targetingRules where rule.matches(context) {
            ...
            case .forceVariant(let forced):
                guard definition.containsVariant(named: forced) else {
                    // Validation rejects this, so reaching it means the ruleset
                    // was built by hand. Fail safe rather than serve a variant
                    // the app has no code path for.
                    return FlagDecision(key: key, variant: failSafe, reason: .malformedDefinition,
                                        rulesetVersion: ruleset.version,
                                        inclusionBucket: inclusionBucket, splitBucket: splitBucket)
                }
                return FlagDecision(key: key, variant: forced, reason: .targetingForced(ruleID: rule.id),
                                    rulesetVersion: ruleset.version,
                                    inclusionBucket: inclusionBucket, splitBucket: splitBucket)
            case .exclude:
                return FlagDecision(key: key, variant: failSafe, reason: .targetingExcluded(ruleID: rule.id),
                                    rulesetVersion: ruleset.version,
                                    inclusionBucket: inclusionBucket, splitBucket: splitBucket)
        ...
        // construction: the bucket does not depend on the ramp, so raising the
        // ramp can only ever *add* identities to the included set. Any
        // implementation that folds the percentage into the hash, or takes the
        // bucket modulo a ramp-derived count, loses this and will yank the
        // feature away from users who already had it.
        let scaledThreshold = (effectiveRollout.value * bucketer.bucketCount) / BasisPoints.max.value
        guard inclusionBucket < scaledThreshold else {
            return FlagDecision(key: key, variant: failSafe, reason: .rolloutExcluded(ruleID: matchedRuleID),
                                rulesetVersion: ruleset.version,
                                inclusionBucket: inclusionBucket, splitBucket: splitBucket)
        }

        let variant = definition.variant(forSplitBucket: splitBucket, bucketCount: bucketer.bucketCount) ?? failSafe
        return FlagDecision(key: key, variant: variant, reason: .rolloutIncluded(ruleID: matchedRuleID),
                            rulesetVersion: ruleset.version,
                            inclusionBucket: inclusionBucket, splitBucket: splitBucket)
    }
}
